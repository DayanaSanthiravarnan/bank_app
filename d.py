...
def Create_New_Account():
    ...
    try:
        ...
        print("\nError writing to account_no.txt: ",e)
        ...
        with open("account_no.txt" , 'w') as account_file:
            for line in lines:
                ...
                if line_part[1] == acc_no:
                    ...
                    date_time = datetime.now().strftime('%d-%m-%Y %A %I:%M %p')
                    with open("transaction.txt", "a") as transaction_file:
                        transaction_file.write(f"Account number,{acc_no},Last balance,{balance},Withdraw,{withdraw_amount},Time,{date_time}\n")
...
def transaction_history():
    ...
    try:
        with open("transaction.txt", "r") as transaction_file:
            print(f"{'Acc_No':<10}{'Ac_No':<10}{'last balance':<15}{'balance':<15}{'Action':<10}{"amount":<15}{"date":}{"time":}\n ")
            for trans_line in transaction_file:
                trans_data = trans_line.strip().split(",")
                if acc_no == trans_data[0]:
                    print(f"{trans_data[0]:<10}{trans_data[1]:<15}{trans_data[2]:<10}{trans_data[3]:<15}{trans_data[4]}{trans_data[5]}{trans_data[6]}{trans_data[7]}\n")
                else:
                    continue
